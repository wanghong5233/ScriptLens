"""剧本 6 维评分（release/v1-mvp，self-contained，零 tag_pipeline 依赖）。

6 维：story / character / concept / emotion / pacing / dialogue。
risk 不在本模块；合规审核走 risk_screener.py / compliance_scorer.py，
落到 ReportPayload.compliance（与 scorecard 平级）。

设计：
- 每维以**规则评分**为骨架（信号来自上游 chain：beat_sheet / character_graph /
  coverage_card / motivation_chain 决策回扫 / reward_events 统计，
  以及 scenes 表本身），不再让 LLM 自己决定档位
- evidence_ref_ids 来自规则锚定的具体场景（beat anchor / 决策回扫 /
  reward 命中 / coverage anchor / 对白密集场），不需要 LLM 二次重试给证据
- LLM 仍参与的是上游 chain（如 character_graph、motivation_chain.score_motivation），
  不再在本模块单独发评分 prompt

历史：
- 原老 5 维 scorer 实现于 commit 264343c（2026-05-20，doc-studio stabilize）
- Batch3（commit e861aa7）一度被「rubric + signal_catalog + tag_pipeline」体系替换，
  但该体系强耦合于全剧抽情节打标签流水线，单部剧报告耗时 30+ 分钟
- 2026-05-29 release/v1-mvp：评分回归 self-contained 规则评分骨架（即本模块），
  仅在原 5 维基础上**新增 dialogue 第 6 维**（rubric §3.6）
- Batch3 时代的 signal_catalog / score_registry / tag_pipeline / decision_aggregator /
  dimension_aggregator / improvement_action_generator / pacing_aggregator /
  evaluation_chain / bundle_extractor / plot_unit_segmenter / character_entity_resolver /
  relationship_candidate_generator / tag_alignment_analyzer 全部进入 dead-code 隔离区，
  下次 cleanup PR 统一清理

------------------------------------------------------------
阈值校准状态：**prototype（demo 期 first-cut 估算）**
------------------------------------------------------------
本文件中所有 numerical thresholds（如 `twist_per_ep >= 0.5`、`reward / 集 >= 1.5`、
`cv <= 0.6`、`mid_ratio >= 0.9`、`dialogue_ratio >= 0.5`）均为基于行业经验的
first-cut 估算，**未经样本回归校准**。
...
- 短剧评分维度的判据本身可量化（节拍完整性 / 反转密度 / OOC 计数 / 题材关键词 / 对白密度），
  让 LLM 决定档位是把可解释性让给幻觉
...
- score_dialogue 需要 全剧 scenes（按行解析「角色：台词」格式）
"""
...
# 对外暴露：改写链和解释链共用同一套 story 评分信号阈值，避免多处硬编码漂移。
STORY_KEY_BEATS = _KEY_BEATS
STORY_TWIST_EVENT_TYPES = _TWIST_EVENT_TYPES
STORY_TWIST_PER_EP_HIGH = _TWIST_PER_EP_HIGH
STORY_TWIST_PER_EP_MID_HIGH = _TWIST_PER_EP_MID_HIGH
STORY_TWIST_PER_EP_MID_LOW = _TWIST_PER_EP_MID_LOW
...
# ============================================================
# 6. score_dialogue —— 对白力（release/v1-mvp 6 维新增）
# ============================================================
#
# rubric §3.6（release/v1-mvp 表述）：对白密度 + 简洁度 + 角色话语分布
#
# 业界参考（docs §2 主线 + 短剧业内）：
# - CML-Bench Dialogue Coherence：可解释指标衡量相邻对白、角色风格、意图行动一致性
# - SkyScript-100M Dialogue Quality：以对白驱动情绪、信息密度、角色辨识度作为评估维度
# - 抖音 2024 短剧 SOP：单场 6-15 行对白 / 单条 ≤ 35 字 / 头部 3 角色话语 ≤ 75% 为合格
#
# 规则信号（不依赖 LLM、不依赖 tag_pipeline；scene.text 按行解析）：
#   - dialogue_ratio       = 对白行数 / 总非空行数（剧本对白比重）
#   - avg_dialogue_len     = 平均单条对白长度（字）
#   - long_dialogue_ratio  = 单条对白 ≥ 80 字的占比（话痨程度）
#   - speaker_top3_share   = 前 3 名角色对白数 / 总对白数（话语集中度）
#
# 对白行识别：行首为 1-6 个中文字符（人名）+ 中/英文冒号 + 台词；
# 与 scene_repo.extract_quote 的 pass-2 对白识别策略保持一致。
#
# 阈值业内基准（短剧场景，2024 业内 SOP first-cut）：
#   - high     : dialogue_ratio ≥ 0.50 且 avg ≤ 35 字 且 long ≤ 0.10 且 top3 ≤ 0.75
#       —— 抖音 2024 SOP「对白驱动 / 角色分散 / 短句快节奏」典型值
#   - mid_high : dialogue_ratio ≥ 0.40 且 avg ≤ 50 字
#       —— 快手 StreamLake「合格」线
#   - mid_low  : dialogue_ratio ≥ 0.25
#       —— 阅文短剧 IP 保底线

import re

_DIALOGUE_LINE_PATTERN = re.compile(r"^([\u4e00-\u9fff]{1,6})\s*[：:]\s*(.+)$")

_DIALOGUE_LONG_LEN = 80
_DIALOGUE_RATIO_HIGH = 0.50
_DIALOGUE_RATIO_MID_HIGH = 0.40
_DIALOGUE_RATIO_MID_LOW = 0.25
_DIALOGUE_AVG_LEN_HIGH = 35
_DIALOGUE_AVG_LEN_MID_HIGH = 50
_DIALOGUE_LONG_RATIO_HIGH = 0.10
_DIALOGUE_SPEAKER_TOP3_HIGH = 0.75
...
def score_dialogue(
    *,
    ...
    total_nonblank = 0
    dialogues: List[Tuple[str, str, str]] = []  # (scene_id, speaker, text)
    for sc in scenes:
        for raw in (sc.text or "").split("\n"):
            ln = raw.strip()
            if not ln:
                continue
            total_nonblank += 1
            match = _DIALOGUE_LINE_PATTERN.match(ln)
            if match is None:
                continue
            ...
            if not speaker or not line_text:
                continue
            dialogues.append((sc.id, speaker, line_text))

    if total_nonblank == 0 or not dialogues:
        return ScoreOutput(
            score=1,
            level="low",
            reason="未识别到任何对白行，对白力极低",
            evidence_ref_ids=[],
        )

    dialogue_ratio = len(dialogues) / total_nonblank
    lens = [len(d[2]) for d in dialogues]
    avg_len = statistics.fmean(lens) if lens else 0.0
    long_count = sum(1 for n in lens if n >= _DIALOGUE_LONG_LEN)
    long_ratio = long_count / len(lens) if lens else 0.0

    speaker_counts: Dict[str, int] = {}
    for _, sp, _ in dialogues:
        speaker_counts[sp] = speaker_counts.get(sp, 0) + 1
    top3 = sorted(speaker_counts.values(), reverse=True)[:3]
    top3_share = sum(top3) / len(dialogues) if dialogues else 1.0

    high = (
        dialogue_ratio >= _DIALOGUE_RATIO_HIGH
        and avg_len <= _DIALOGUE_AVG_LEN_HIGH
        and long_ratio <= _DIALOGUE_LONG_RATIO_HIGH
        and top3_share <= _DIALOGUE_SPEAKER_TOP3_HIGH
    )
    mid_high = (
        dialogue_ratio >= _DIALOGUE_RATIO_MID_HIGH
        and avg_len <= _DIALOGUE_AVG_LEN_MID_HIGH
    )
    mid_low = dialogue_ratio >= _DIALOGUE_RATIO_MID_LOW
    score, level = _score_from_signals(high=high, mid_high=mid_high, mid_low=mid_low)

    reason = (
        f"对白占比 {dialogue_ratio:.0%}；平均 {avg_len:.0f} 字/句；"
        f"长台词 ≥ {_DIALOGUE_LONG_LEN} 字占 {long_ratio:.0%}；"
        f"前 3 角色话语占 {top3_share:.0%}"
    )

    # evidence: 取前 2 个对白最密集的场（最能展示对白特点）
    scene_dialogue_count: Dict[str, int] = {}
    for sid, _, _ in dialogues:
        scene_dialogue_count[sid] = scene_dialogue_count.get(sid, 0) + 1
    dense_scenes = sorted(scene_dialogue_count.items(), key=lambda kv: -kv[1])
    evidence_ref_ids = [sid for sid, _ in dense_scenes[:2]]
