...
from eval.stability.segmentation_runner import run_segmentation_repeat
...
def test_segmentation_runner_stable(monkeypatch, tmp_path) -> None:
    monkeypatch.setattr(
        "eval.stability.segmentation_runner._scene_index",
    ...
    monkeypatch.setattr("eval.stability.segmentation_runner.segment_plot_units", _fake_segment_plot_units)
    exp_dir = ExperimentDir.create(
        ...
        scripts=["script-1"],
        cache_disabled=True,
        run_id="segmentation_stable",
        root=tmp_path,
    )

    report = asyncio.run(
        run_segmentation_repeat(
            "script-1",
            seed=42,
            n_repeats=5,
            tag_set_ver="v2.0.0",
...
def test_segmentation_runner_unstable(monkeypatch, tmp_path) -> None:
    monkeypatch.setattr(
        "eval.stability.segmentation_runner._scene_index",
        lambda script_id, engine: (
            ["s1", "s2", "s3", "s4", "s5"],
